...
public class NadeLevelSelection : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (button != null)
        {
            ...
            button.onClick.AddListener(PressNadeSelection);
        }
    ...
    public void UpdateLevelStatus()
    {
        ...
        if (PlayerPrefs.GetInt("NLv" + previousLevelNum.ToString()) >= 0 &&
        PlayerPrefs.GetInt("NLevel" + previousLevelNum.ToString() + "_Win") == 1)//If star >= 0 and win, next level can play
        {
            unlocked = true;
        }
    }
    public void UpdateLevelImage()
    {
        ...
        else //level open
        {
            ...
            for (int i = 0; i < PlayerPrefs.GetInt("NLv" + gameObject.name); i++)
            {
                stars[i].gameObject.GetComponent<Image>().sprite = starSprite;
            }
        }
    }
    public void PressNadeSelection()
    {
        Debug.Log("Button Pressed for Hostage Level!");
        if (unlocked)
        {
            PlayerPrefs.SetInt("SelectedNadeLevel", levelId);
